    ...
    def __call__(self, task_dict):
        ...
        for task in task_dict:
            ...
            if ijhw is None:
                ...
                try:
                    i = random.randint(0, task_dict[task].size[1] - self.input_size)
                    j = random.randint(0, task_dict[task].size[0] - self.input_size)
                    ijhw =(i,j,self.input_size, self.input_size)
                except:
            ...
            if task == "UAV_RGB":
                ...
                task_dict[task] = self.color_jitter(task_dict[task])
            ...
            if flip:
                ...
                task_dict[task] = TF.rotate(task_dict[task],rotate_angle)
        ...
        for task in task_dict:	
            if task in ['depth']:	
            ...
            elif task in ['UAV_RGB']:	
                img = TF.to_tensor(task_dict[task])	
                # img = TF.normalize(img, mean=self.uav_rgb_mean, std=self.uav_rgb_std)
                img = TF.normalize(img, mean=self.rgb_mean, std=self.rgb_std)	
            elif task in ['Sat_RGB']:	
                img = TF.to_tensor(task_dict[task])	
                ...
                # image_array_sat =img.numpy().transpose(1, 2, 0)
                # arr =image_array_sat
                # arr = (arr - np.min(arr)) / (np.max(arr) - np.min(arr))
                # arr = (arr * 255).astype(np.uint8)
                # image_sat = Image.fromarray(arr)
                # image_sat.save(str(rotate_angle)+'usat.png')


                # img = TF.normalize(img, mean=self.sat_rgb_mean, std=self.sat_rgb_std)
                img = TF.normalize(img, mean=self.rgb_mean, std=self.rgb_std)	

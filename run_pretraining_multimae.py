...
def main(args):
    ...
    if args.extra_norm_pix_loss:
        for k in list(args.in_domains):
            if 'RGB' in k:
                tasks_loss_fn['norm_'+k] = DOMAIN_CONF[k]['loss'](patch_size=args.patch_size,
                                                                       stride=DOMAIN_CONF[k]['stride_level'],
                                                                       norm_pix=True)
...
def train_one_epoch(model: torch.nn.Module, data_loader: Iterable, tasks_loss_fn: Dict[str, torch.nn.Module],
                    ...
                    extra_norm_pix_loss: bool = False, fp32_output_adapters: List[str] = [], out_path = 'output/'):
    ...
    for step, (x, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            ...
            #TODO: Get masks from the other modality (RGB) as well.
            if extra_norm_pix_loss:
                for k in list(in_domains):
                    if 'RGB' in k:
